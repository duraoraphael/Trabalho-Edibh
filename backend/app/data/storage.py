...
class PostgresStore:
    ...
    def initialize(self) -> None:
        ...
        self._execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS telefone TEXT")
        self._execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS foto_url TEXT")
        self._execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS last_access TIMESTAMPTZ")
        self._execute(
            """
    ...
    def _seed_users_from_json(self) -> None:
        existing = self._execute("SELECT COUNT(*) AS count FROM users", fetch_one=True)
        ...
        for user in users:
            self._execute(
                ...
                VALUES (%(id)s, %(name)s, %(email)s, %(role)s, %(is_active)s, %(password_hash)s)
    ...
    def get_user_by_email(self, email: str) -> dict[str, Any] | None:
        row = self._execute("SELECT * FROM users WHERE email = %(email)s LIMIT 1", {"email": email}, fetch_one=True)
        if not row:
            return None
        return dict(row)
    ...
    def create_user(self, user_data: dict[str, Any]) -> dict[str, Any]:
        row = self._execute(
            ...
            VALUES (%(name)s, %(email)s, %(role)s, %(is_active)s, %(password_hash)s)
            RETURNING id, name, email, role, is_active, telefone, foto_url, last_access, password_hash
            """,
            {
                "name": user_data["name"],
                "email": user_data["email"],
    ...
    def authenticate_user(self, email: str, password: str) -> dict[str, Any] | None:
        ...
        if not verify_password(password, user.get("password_hash", "")):
            return None
        self._execute("UPDATE users SET last_access = NOW() WHERE id = %(id)s", {"id": user["id"]})
        user["last_access"] = datetime.now(timezone.utc).isoformat()
        return user

    def list_users(self, filters: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        query = "SELECT id, name, email, role, is_active, telefone, foto_url, last_access FROM users"
        conditions: list[str] = []
        params: dict[str, Any] = {}
        if filters:
            if filters.get("role"):
    ...
    def get_user_by_id(self, user_id: int) -> dict[str, Any] | None:
        row = self._execute("SELECT * FROM users WHERE id = %(id)s LIMIT 1", {"id": user_id}, fetch_one=True)
    ...
    def update_user(self, user_id: int, updates: dict[str, Any]) -> dict[str, Any]:
        ...
        if updates.get("password"):
            self._execute("UPDATE users SET password_hash=%(password_hash)s WHERE id=%(id)s", {"id": user_id, "password_hash": hash_password(updates["password"])})
        updated = self.get_user_by_id(user_id)
        return updated or {}

    def delete_user(self, user_id: int) -> None:
        self._execute("DELETE FROM users WHERE id = %(id)s", {"id": user_id})
